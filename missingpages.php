...
function RefreshSomeEventListener() {
	$( ".savestudentsattendance" ).on( "click", function() {
		...
		$.ajax({
		    ...
		    success: function (response) {
				var error = response["guardar"];
				var error2 = response["omegatoken2"];
				var moodleurl = "<?php echo $CFG->wwwroot;?>";
				$('#loader').hide();
				$("#alerthelp").hide();
				$("#inputs").html('<div class="alert alert-success" role="alert" style="float:left; margin-top:5%;">'+error+error2+'</div>');
