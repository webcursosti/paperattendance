...
if ($action == "edit") {
	if ($sesspageid == null) {
		...
		if ($session = $DB->get_record("paperattendance_sessionpages", array("id" => $sesspageid))){
			...
			$inputs .= html_writer::div('<button type="submit" id="confirm" class="btn btn-default">Continuar</button>',"form-group", array("style"=>"float:right; margin-right:5%;"));
		...
		$.ajax({
		    ...
		    success: function (response) {
		        ...
		        if (error != 0){
		    		...
		    		$("#pdfviewer").append('<button class="btn btn-info savestudentsattendance" style="float:right; width:30%; margin-right:10%; padding-top:5%;">Guardar Asistencia</button>');
