...
if($resources = $DB->get_records_sql($sqlunreadpdfs, array())){
	...
	if (count($pagesWithErrors) > 0){
		paperattendance_sendMail($pagesWithError, null, $uploaderobj->id, $uploaderobj->id, null, null, "nonprocesspdf", null);
		$admins = get_admins();
		foreach ($admins as $admin){
			paperattendance_sendMail($pagesWithError, null, $admin->id, $admin->id, null, null, "nonprocesspdf", null);
		}
	}
